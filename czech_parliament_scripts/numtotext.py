...
for sentence in sys.stdin:
    if len(sentence.strip()) == 0:
        continue
#.replace(",", "")
    sentence = sentence.replace("%", " PROCENT").replace(
        "...", ".").replace("–", "").replace("-", "").replace("‘", "")\
            .replace("§","PARAGRAFU ").replace("písm.", "písmena ").replace("č.","číslo ")\
                .replace("/"," LOMENO ").replace("Ing.","Inženýr ").replace("Sb.","sbírky ")\
                    .replace("Bc.", "Bakalář ").replace("písm.", "písmeno ").replace("END","")\
                        .replace("START","").replace("sttm","").replace("ettm","").replace("*","")\
                            .replace("\"","").replace("mld.","miliard").replace("mil.","milionů")\
                                .replace("(","").replace(")","").replace("atd.","a tak dále")\
                                    .replace("tzv.","takzvaný").replace("resp.", "respektivě")\
                                        .replace("ČR.", "Česká Republika").replace("Čr.", "Česká Republika")
    if len(sentence.strip()) < 11:
        sentence.replace(",","")
    ...
    for i, word in enumerate(arr_sen):
        ...
        if word == ":":
            flag = True
            ind = i
        # This deals with cases such as B3 A10,...
        if word[1:].isdigit():
            arr_sen[i] = str(word[0]) + " " + str(word[1:])
            
        if len(word.split(".")) == 2 or len(word.split(".")) == 3:

            if len(word.split(".")[0]) > 0 and len(word.split(".")[1]) >0:
                arr_sen[i] = str(word.split(".")[0] + " " + word.split(".")[1])
            else:
                pass
        if len(word.split(",")) == 2:
            
            if len(word.split(",")[0]) > 0 and len(word.split(",")[1]) > 0:
                arr_sen[i] = str(word.split(",")[0] + " celých " + word.split(",")[1])
            else:
                pass
        if len(word.split(":")) == 2:
            if len(word.split(":")[0]) > 0 and len(word.split(":")[1]) > 0:
                arr_sen[i] = str(word.split(":")[0] + " ku " + word.split(":")[1])
            else:
                pass
        if len(word.split("-")) == 2:
            if len(word.split("-")[0]) > 0 and len(word.split("-")[1]) > 0:
                arr_sen[i] = str(word.split("-")[0] + " až " + word.split("-")[1]) 
            else:
                pass

        if word.isdigit() or (word[:-1].isdigit() and int(word[:-1]) > 50) or (word[:-1].isdigit() and word[-1] != "."):
            ...
            if lenght == 2 and word[0] == "1":
                ...
                if int(word[-2:]) < 11 or int(word[-2:]) > 19:
                    ...
                    arr_sen[i] = str(stovky(word[0])) + " " + str(desitky(word[1])) + \
                        " " + \
                        str(jednotky(word[2])) + " TISÍC " + str(stovky(word[3])) + " " + str(jedenact_az_devatenact(word[-2:]))
                ...
                if int(word[-2:]) < 11 or int(word[-2:]) > 19:
                    ...
                    arr_sen[i] = "MILION " + str(stovky(word[1])) + " " + str(desitky(word[2])) + \
                        " " + \
                        str(jednotky(word[3])) + " TISÍC " + str(stovky(word[4])) + " " +str(jedenact_az_devatenact(word[-2:]))
            ...
            continue
    if flag:
        arr_sen = arr_sen[ind+1:]

    new_sentence = " ".join(arr_sen).strip()
    new_sentence = re.sub(' +', ' ', new_sentence)
    new_sentence = new_sentence.strip().upper()
