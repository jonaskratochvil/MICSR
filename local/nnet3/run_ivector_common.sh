...
test_sets="dev test"
gmm=tri2                # This specifies a GMM-dir from the features of the type you're training the system on;
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$mfcc_cmd" \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$mfcc_cmd" --nj 30 \
    --num-frames 700000 \
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$mfcc_cmd" --nj 10 --num-processes 2 --num-threads 4 \
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --stage 0 --cmd "$train_cmd" --nj $nj \
